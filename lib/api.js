...
class StellantisApi {
    ...
    _log(level, msg) {
        const logger = this.tm?.adapter?.log;
    ...
    _formatAxiosError(err) {
        ...
        return `status=${status || "n/a"} ${statusText || ""}${responseText}`.trim();
    }
    ...
    async _get(path, params = {}) {
        ...
        try {
            ...
            this._log("warn", `[API] GET ${path} failed: ${this._formatAxiosError(err)}`);
            throw err;
        }
    }
    ...
    async _post(path, data = {}) {
        ...
        try {
            ...
            this._log("warn", `[API] POST ${path} failed: ${this._formatAxiosError(err)}`);
            throw err;
        }
    }
